...
<div class="row"><div id="wow"></div>
	...
	?>
	<h3 class="page-header" style="padding-left:5px;"><i class="fa fa-question-circle"></i> Soal <?php echo $_GET['soal2']-60; ?>/60</h3>
	<div class="col-md-4" style="border-right: 1px solid #ddd">
		<?php
			for($i=1; $i<=60; $i++){
				$isNULL = "isset";
				if(!isAnswer(($i+60), $info['id_peserta'])){
					$isNULL = "default";
				}
				if($_GET['soal2']!=($i+60))
	...
	<div class="col-md-8">
		<?php 
			...
			$soal = mysqli_fetch_assoc(mysqli_query($conn, "SELECT pertanyaan FROM isc_soal WHERE no_soal = ".addslashes(strip_tags($_GET['soal2'])).""));
		...
		<form method="post" id="soalisc">
			...
			<div class="form-group">
				<?php if($_GET['soal2']!=61) { $back = $_GET['soal2']-1; ?>
				<input type="button" name="back" class="btn btn-primary" onclick="cekAns(<?php echo $back-60; ?>)" value="Back"/>
				<?php } ?>
				<?php $next = $_GET['soal2']+1; if($_GET['soal2']==120) {  ?>
				<input type="button" name="btn_submit_all" id="btn_submit" class="btn btn-primary" onclick="cekAns(<?php echo $next-60; ?>)" value="Finish"/>
				<?php }else{ ?>
				<input type="button" name="btn_submit" onclick="cekAns(<?php echo $next-60; ?>)" id="btn_submit" class="btn btn-primary" value="Next"/>
				<?php } ?>
			</div>
		</form>
	</div>
	<?php }else if($_GET['soal2'] == "summary"){ print('<h3 class="page-header" style="padding-left:5px;"><i class="fa fa-file-o"></i> Summary</h3>'); include "page/summary.php"; } ?>
</div>
	<script>
		$('#soalisc').bind("keypress", function(e){
			if(e.which == 13){
				cekAns(<?php echo $next-60; ?>);
			}
		});
	</script>
	<script>
		function cekAns(pindah){
			var no_soal = <?php echo $_GET['soal2'] ?>;
	...
	<script>
		...
		$(document).everyTime('1s', function(i){
			$.ajax({
			  url: "<?php echo $inclusion; ?>php/ajaxtime2.php",
			  success: function(data){
			  	if(data != "timesup"){
			  		$('#wow').html("Tersisa: " + Math.floor(data / 3600) + " jam " + Math.floor(data % 3600 / 60) + " menit " + data % 60 + " detik");
			  	}else{
